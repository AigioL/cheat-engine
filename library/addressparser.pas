...
//    c:=@memorybrowser.lastdebugcontext;
  ...
  if (length(tmps)=3) and (tmps[1] in ['X','Y']) and (copy(tmps,2,2)='MM') then
  begin
    inc(ch,3);
    xregnrstr:='';
    while (ch<=length(str)) and (str[ch] in ['0'..'9']) do
    begin
    ...
    setlength(values,length(values)+1);
    values[length(values)-1]:=pptruint(@c^.{$ifdef cpu64}FltSave.XmmRegisters[xregnr]{$else}ext.XMMRegisters.LegacyXMM[xregnr]{$endif})^;
    exit;
  end;
...
begin
  ...
  begin
    result:=symhandler.getaddressfromname(s,false,haserror, SpecialContext);
