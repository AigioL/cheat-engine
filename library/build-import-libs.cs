...
internal static class Program
{
    ...
    private static string GetStripPath()
    {
        return FindFirstExistingFile(
                   new[]
                   {
                       FindExecutableInPath("strip.exe"),
                       @"C:\lazarus\fpc\3.2.2\bin\x86_64-win64\strip.exe"
                   },
                   "strip.exe",
                   new[] { @"C:\lazarus\fpc" })
               ?? throw new InvalidOperationException("Unable to locate strip.exe.");
    }
    ...
    private static void StripBinary(string stripExe, string dllPath)
    {
        if (!File.Exists(dllPath))
        {
            throw new InvalidOperationException($"Expected build output was not found: {dllPath}");
        }

        var exitCode = RunProcess(
            stripExe,
            new[] { "--strip-all", dllPath },
        ...
        if (exitCode != 0)
        {
            throw new InvalidOperationException($"strip.exe failed for {dllPath}.{Environment.NewLine}{standardError}".Trim());
        }
    }
