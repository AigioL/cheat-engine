...
uses jwawindows, windows, LCLIntf,Classes,SysUtils,messages,
Graphics,
ComCtrls,
{reinit, }
assemblerunit,
...
{$endif}
{$endif}
 math, shellapi, ProcessHandlerUnit, controls, shlobj, ActiveX, strutils, LazUTF8;
...
begin
  ...
  result:=((mbi.Protect and PAGE_EXECUTE_READWRITE)=PAGE_EXECUTE_READWRITE) or
...
begin
  ...
  if ths=INVALID_HANDLE_VALUE then exit;
...
begin
  ...
  ZeroMemory(@BAddress, sizeof(BAddress));
...
begin
  ...
  CheatEngineDir:=ExtractFilePath(ModulePath);
...
begin
  result:=false;
  if VirtualProtectEx(processhandle, pointer(address), size, PAGE_EXECUTE_READWRITE, original) then
  begin
    result:=writeprocessmemory(processhandle,pointer(address),buffer,size,written);
    size:=written;
...
function ConvertHexStrToRealStr(const s: string): string;
begin
  result:=parsers.ConvertHexStrToRealStr(s);
end;

function IntToHexSigned(v: INT64; digits: integer): string;
begin
  result:=parsers.IntToHexSigned(v, digits);
end;

function HexStrToInt(const S: string): Integer;
begin
  result:=parsers.HexStrToInt(S);
end;

function HexStrToInt64(const S: string): Int64;
begin
  result:=parsers.HexStrToInt64(S);
end;
