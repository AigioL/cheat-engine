...
//    cAddressFile,cMemoryFile: TCompressionStream;
  ...
  private
    ...
    fStartaddress: ptruint; //start for the whole scan
    fStopaddress: ptruint; //stop of the whole scan
  ...
  public
    ...
    function GetProgress(var totaladdressestoscan:qword; var currentlyscanned: qword):integer; overload;
    function GetProgress(var totaladdressestoscan:qword; var currentlyscanned: qword; var resultsfound: qword):integer; overload;
    ...
    procedure NextScan(scanOption: TScanOption; roundingtype: TRoundingType; scanvalue1, scanvalue2: string; hexadecimal,binaryStringAsDecimal, unicode, casesensitive,percentage,compareToSavedScan: boolean; savedscanname: string); overload; //next scan, determine what kind of scan and give to firstnextscan/nextnextscan
  ...
  published
    ...
    property VarType: TVariableType read currentVariableType write setVariableType;
    property VariableType: TVariableType read currentVariableType write setVariableType;
    ...
    property CustomType: TCustomType read currentCustomType write currentCustomType;
    property lastScanWasRegionScan: boolean read GetLastScanWasRegionScan;
    property isUnicode: boolean read stringUnicode;
    property isHexadecimal: boolean read fHexadecimal;
...
var i: integer;
  ...
  currentaddress: ptruint;
begin
  currentaddress:=fscanner.currentAddress;
  result:=true;
  ...
  begin
    ...
    case groupdata[i].vartype of
      ...
      begin
        ...
        inc(currentaddress, groupdata[i].bytesize);
      end;
      ...
      begin
        ...
        begin
          s:=groupdata[i].customType.ConvertDataToString(newvalue, currentaddress);
          result:=groupdata[i].wildcard or testString(pchar(s), @groupdata[i].value[1]);
        end
        else
        if groupdata[i].customType.scriptUsesFloat then
        begin
          f:=groupdata[i].customType.ConvertDataToFloat(newvalue, currentaddress);
          result:=groupdata[i].wildcard or ((f>groupdata[i].minfvalue) and (f<groupdata[i].maxfvalue));
        end
        else
          result:=groupdata[i].wildcard or (groupdata[i].customType.ConvertDataToInteger(newvalue, currentaddress)=groupdata[i].valuei);

        inc(newvalue, groupdata[i].customType.bytesize);
        inc(currentaddress, groupdata[i].customType.bytesize);
      end;
...
begin
  ...
  begin
    if ct.ConvertDataToInteger(current, fscanner.currentAddress+i)=value then
    begin
      startoffset:=i+1;
      result:=true;
      exit;
...
begin
  ...
  while i<(blocksize-ct.bytesize-1) do
  begin
    f:=ct.ConvertDataToFloat(current, fscanner.currentAddress+i);
    ...
    case groupdata[i].vartype of
      ...
      begin
        ...
        begin
          ...
          if groupdata[i].customtype.scriptUsesString then
            result:=StringScan(pchar(groupdata[i].customtype.ConvertDataToString(newvalue, fscanner.currentAddress+currentoffset)), newvalue, currentoffset)
          else
          if groupdata[i].customtype.scriptUsesFloat then
...
begin
  result:=RoundTo(customType.ConvertDataToFloat(newvalue, currentAddress),-floataccuracy)=RoundTo(customType.ConvertDataToFloat(oldvalue, currentAddress)+svalue,-floataccuracy);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        currentAddress:=base+ptruint(p)-ptruint(buffer);
        if checkroutine(p,savedscanhandler.getpointertoaddress(currentAddress,valuetype,nil )) then //found one
          StoreResultRoutine(currentAddress,p);
...
begin
  ...
  begin
    ...
    if scanvalue1='' then raise exception.Create(rsPleaseFillSomethingIn);
     ...
     if (variableType in [vtByte,vtWord,vtDWord,vtQword,vtAll,vtCustom]) and
       ((variableType<>vtCustom) or (customType=nil) or (customType.scriptUsesString=false)) then
    begin
      //parse scanvalue1
      try
        if hexadecimal then
     ...
     if percentage or ((variableType in [vtsingle,vtDouble,vtAll, vtCustom]) and
       ((variableType<>vtCustom) or (customType=nil) or (customType.scriptUsesString=false))) then
    begin
      try
        if hexadecimal then
    ...
    if (variableType = vtString) or ((variableType=vtCustom) and (customtype<>nil) and customtype.scriptUsesString) then
    begin
...
function TMemscan.hasError: boolean;
begin
  result:=false;
  if scancontroller<>nil then
...
var resultsfound: qword;
begin
  result:=GetProgress(totaladdressestoscan, currentlyscanned, resultsfound);
end;
...
begin
  fRoundingtype:=roundingtype;
  fScanvalue1:=scanvalue1;
  fScanvalue2:=scanvalue2;
  fHexadecimal:=hexadecimal;
  fBinaryStringAsDecimal:=binaryStringAsDecimal;
  fUnicode:=unicode;
  fCasesensitive:=casesensitive;
  fScanOption:=scanOption;
  fPercentage:=percentage;
  fCompareToSavedScan:=compareToSavedScan;
  fSavedScanName:=savedscanname;
...
begin
  ...
  scanController.scanOption:=fScanOption;
  scanController.compareToSavedScan:=fCompareToSavedScan;
  scanController.savedscanname:=fSavedScanName;
  scanController.variableType:=CurrentVariableType;
  scancontroller.customType:=customtype;


  scanController.roundingtype:=fRoundingtype;
  ...
  scanController.scanValue1:=fScanvalue1; //usual scanvalue
  scanController.scanValue2:=fScanValue2; //2nd value for between scan
  scanController.startaddress:=self.startaddress;
  scanController.stopaddress:=self.stopaddress;

  scancontroller.hexadecimal:=fHexadecimal;
  scancontroller.binaryStringAsDecimal:=fBinaryStringAsDecimal;
  scancontroller.unicode:=fUnicode;
  scancontroller.casesensitive:=fCasesensitive;
  scancontroller.percentage:=fPercentage;
...
begin
  ...
  fCompareToSavedScan:=false;
  fSavedScanName:='';
  fFastscanParameter:=fastscanparameter;
...
begin
  ...
  if (currentVariableType=vtCustom) and (currentCustomType=nil) then
  ...
  if fFastscanParameter<>'' then
    self.fastscanalignment:=strtoint('$'+fFastscanParameter)
  else
    self.fastscanalignment:=1;

  self.fastscandigitcount:=length(fFastscanParameter);
  ...
  scanController.fastscanalignment:=fastscanalignment;
  scanController.fastscanmethod:=self.fastscanmethod;
  scancontroller.fastscandigitcount:=fastscandigitcount;
...
begin
  ...
  fLastscantype:=stNewScan;
  fRoundingtype:=rtRounded;
  fScanvalue1:='';
  fScanvalue2:='';
  fHexadecimal:=false;
  fBinaryStringAsDecimal:=false;
  fUnicode:=false;
...
begin
  ...
  fScanResultFolder:=usedtempdir+ScanResultsRootFolderName+pathdelim;
  ...
  InuseFile:=TFilestream.Create(fScanResultFolder+pathdelim+'inuse.lock',fmCreate or fmShareExclusive);
...
begin
  ...
  //check for orphaned scan folders left behind by older instances
      ...
      if FindFirst(usedtempdir+ScanResultsRootFolderName+pathdelim+'{*}',  faDirectory , info)=0 then
      begin
        repeat
          ...
          begin
            ...
            begin
              //if found, delete them when no active lock is present
              f:=usedtempdir+ScanResultsRootFolderName+pathdelim+info.name;

              if (FileExists(f+pathdelim+'inuse.lock')=false) or DeleteFile(f+pathdelim+'inuse.lock') then
                deletefolder(f);
            end;
          end;
