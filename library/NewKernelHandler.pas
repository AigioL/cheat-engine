...
procedure CloseFileAsMemoryHandle;
begin
...
begin
  newfilehandle:=CreateFile(pchar(filename),GENERIC_READ	or GENERIC_WRITE,FILE_SHARE_READ or FILE_SHARE_WRITE,nil,OPEN_EXISTING,FILE_FLAG_RANDOM_ACCESS,0);
  if (newfilehandle=0) or (newfilehandle=INVALID_HANDLE_VALUE) then
    raise exception.create(Format(rsCouldnTBeOpened, [filename]));

  if usefileasmemory then
    CloseFileAsMemoryHandle;
...
begin
  ...
  UsePhysical:=true;
  Usephysicaldbvm:=false;
  if usefileasmemory then CloseFileAsMemoryHandle;
  usefileasmemory:=false;
...
function NoEnumDeviceDrivers(lpImageBase: LPLPVOID; cb: DWORD; var lpcbNeeded: DWORD): BOOL; stdcall;
begin
  if (lpImageBase=nil) and (cb=0) then;

  lpcbNeeded:=0;
  result:=false;
end;

function NoGetDeviceDriverBaseNameA(ImageBase: LPVOID; lpBaseName: LPSTR; nSize: DWORD): DWORD; stdcall;
begin
  if ImageBase=nil then;

  if (lpBaseName<>nil) and (nSize>0) then
    lpBaseName[0]:=#0;

  result:=0;
end;

function NoGetDeviceDriverFileName(ImageBase: LPVOID; lpFilename: LPTSTR; nSize: DWORD): DWORD; stdcall;
begin
  if ImageBase=nil then;

  if (lpFilename<>nil) and (nSize>0) then
    lpFilename[0]:=#0;

  result:=0;
end;

function NoPrintWindow(hwnd: HWND; hdcBlt: HDC; nFlags: UINT): BOOL; stdcall;
begin
  if (hwnd=0) and (hdcBlt=0) and (nFlags=0) then;
...
begin
  if ReturnedLength<>nil then
    ReturnedLength^:=0;

  if (Buffer<>nil) and (ReturnedLength<>nil) and (ReturnedLength^>=sizeof(Buffer^)) then
...
initialization
  ...
  begin
    ...
    GetDevicedriverFileName:=GetProcAddress(psa,'GetDeviceDriverFileNameA');
  end;

  if not assigned(EnumDeviceDrivers) then
    EnumDeviceDrivers:=@NoEnumDeviceDrivers;

  if not assigned(GetDevicedriverBaseNameA) then
    GetDevicedriverBaseNameA:=@NoGetDeviceDriverBaseNameA;

  if not assigned(GetDevicedriverFileName) then
    GetDevicedriverFileName:=@NoGetDeviceDriverFileName;

  u32:=loadlibrary('user32.dll');
  if u32<>0 then
    PrintWindow:=GetProcAddress(u32,'PrintWindow');

  if not assigned(PrintWindow) then
    PrintWindow:=@NoPrintWindow;
