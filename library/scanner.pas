...
begin
  if state<>nil then
  begin
    if (state^.foundlist<>nil) and (state^.foundlist<>foundlist) then
      FreeAndNil(state^.foundlist);

    if (state^.memscan<>nil) and (state^.memscan<>memscan) then
      FreeAndNil(state^.memscan);
