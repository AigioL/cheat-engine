...
function StrToFloatEx(const value: string): double;
var
  fs: TFormatSettings;
begin
  try
  ...
  begin
    try
      ...
      begin
        ...
        begin
          if (VarType=vtCustom) and (customtype<>nil) and customtype.scriptUsesString then exit;
          oldvalue:=StrToQWordEx(getvalue);
          increasevalue:=StrToQWordEx(value);
      ...
      begin
        ...
        begin
          if customtype.scriptUsesString then
            result:=customtype.ConvertDataToString(buf, realAddress)
          else
          if customtype.scriptUsesFloat then
          begin
            if showashex then
            begin
              f:=customtype.ConvertDataToFloat(buf, realAddress);
...
begin
  ...
  if fShowAsHex and (not (vartype in [vtSingle, vtDouble, vtByteArray, vtString] )) and (not ((vartype=vtCustom) and (customtype<>nil) and customtype.scriptUsesString)) then
  begin
  ...
  try
    ...
    case VarType of
      ...
      begin
        ...
        Begin
          if customtype.scriptUsesString then
            customtype.ConvertStringToData(pchar(currentValue), pb, realAddress)
          else
          if customtype.scriptUsesFloat then
          begin
            ...
            begin
              temp:=StrToQWordEx(currentvalue);
              customtype.ConvertFloatToData(psingle(@temp)^, ps, realAddress)
            end;
          end
          else
            customtype.ConvertIntegerToData(StrToQWordEx(currentValue), pdw, realAddress);
